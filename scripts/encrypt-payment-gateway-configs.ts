...
import { decryptPaymentConfig, encryptPaymentConfig, splitPaymentConfig } from '@/lib/payment-config-crypto'
import { eq } from 'drizzle-orm'

async function main() {
  dotenv.config({ path: '.env.local' })
  const { db } = await import('@/db')
  const { paymentGatewayConfigs, paymentGatewayCredentials } = await import('@/db/schema')
  ...
  for (const row of rows) {
    ...
    const { publicConfig, secretConfig } = splitPaymentConfig(decryptPaymentConfig(row.config))
...
main().catch((error) => {
  console.error('Payment gateway migration failed:', error instanceof Error ? error.message : error)
  process.exitCode = 1
})
