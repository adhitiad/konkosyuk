...
async function seed() {
  ...
  await db.transaction(async (tx) => {
    ...
    if (!kostMelati) {
      const melati = await tx.insert(schema.properties).values({
        id: randomUUID(),
    ...
    if (!kostMerdeka) {
      const merdeka = await tx.insert(schema.properties).values({
        id: randomUUID(),
        ownerId: emailToUser.get("owner1@booking.com")!.id,
    ...
    if (!kontrakanKeluarga) {
      const keluarga = await tx.insert(schema.properties).values({
        id: randomUUID(),
    ...
    for (const unit of melatiUnits) {
      if (!unitMap.has(unit.name)) {
        ...
        console.log(`  ✅ Created unit: ${unit.name} (Kost Melati) - Rp${unit.price}`)
      } else {
    ...
    for (const unit of merdekaUnits) {
      if (!unitMap.has(unit.name)) {
        ...
        console.log(`  ✅ Created unit: ${unit.name} (Kost Merdeka) - Rp${unit.price}`)
      } else {
        console.log(`  ⏭️  Skipped unit: ${unit.name} (already exists)`)
      }
    }
    ...
    for (const unit of keluargaUnits) {
      if (!unitMap.has(unit.name)) {
        ...
        console.log(`  ✅ Created unit: ${unit.name} (Kontrakan Keluarga) - Rp${unit.price}`)
      } else {
        console.log(`  ⏭️  Skipped unit: ${unit.name} (already exists)`)
      }
    }
