...
export async function checkFraudFlags(userId: string, amount: number): Promise<FraudCheckResult> {
  ...
  if (failedCount > 3) {
    return { isBlocked: true, reason: 'Terlalu banyak percobaan gagal', requiresManualReview: false }
  }
