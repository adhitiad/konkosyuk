...
export class PaymentGatewayManager {
  ...
  async createPayment(provider: GatewayProvider, request: PaymentRequest): Promise<PaymentResponse> {
  ...
  async checkStatus(provider: GatewayProvider, transactionId: string): Promise<PaymentResponse> {
    this.validateConfig(provider)
  ...
  private async createDokuPayment(request: PaymentRequest): Promise<PaymentResponse> {
    const config = this.configs.get("doku")!
    const baseUrl = config.baseUrl || "https://api.doku.com"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.post<DokuCreateResponse & { message?: string }>(
      ...
      const responseData = (error as any)?.response?.data as Record<string, unknown> | undefined
      throw new PaymentGatewayError("doku", message, status, responseData)
    }
  }

  private async createIpaymuPayment(request: PaymentRequest): Promise<PaymentResponse> {
    const config = this.configs.get("ipaymu")!
    const baseUrl = config.baseUrl || "https://api.ipaymu.com/api/v2"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.post<IpaymuCreateResponse & { message?: string }>(
        `${baseUrl}/transaction`,
        {
          clientId: config.clientId,
      ...
      if (data.status !== "success") {
        throw new PaymentGatewayError("ipaymu", data.message || "Payment creation failed", undefined, data as unknown as Record<string, unknown>)
      }
  ...
  private async createNicepayPayment(request: PaymentRequest): Promise<PaymentResponse> {
    const config = this.configs.get("nicepay")!
    const baseUrl = config.baseUrl || "https://api.nicepay.co.id"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.post<NicepayCreateResponse & { message?: string }>(
        `${baseUrl}/api/v1/payment/create`,
        {
          clientId: config.clientId,
          merchantCode: config.merchantCode,
  ...
  private async checkDokuStatus(transactionId: string): Promise<PaymentResponse> {
    const config = this.configs.get("doku")!
    const baseUrl = config.baseUrl || "https://api.doku.com"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.get<DokuStatusResponse & { message?: string }>(
  ...
  private async checkIpaymuStatus(transactionId: string): Promise<PaymentResponse> {
    const config = this.configs.get("ipaymu")!
    const baseUrl = config.baseUrl || "https://api.ipaymu.com/api/v2"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.get<IpaymuStatusResponse & { message?: string }>(
      ...
      if (data.status !== "success") {
        throw new PaymentGatewayError("ipaymu", data.message || "Status check failed", undefined, data as unknown as Record<string, unknown>)
      }
  ...
  private async checkNicepayStatus(transactionId: string): Promise<PaymentResponse> {
    const config = this.configs.get("nicepay")!
    const baseUrl = config.baseUrl || "https://api.nicepay.co.id"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.get<NicepayStatusResponse & { message?: string }>(
        `${baseUrl}/api/v1/payment/status/${encodeURIComponent(transactionId)}`,
        {
          headers: {
            Authorization: `Bearer ${config.secretKey}`,
  ...
  private async refundDoku(transactionId: string, amount?: number, reason?: string): Promise<PaymentResponse> {
    const config = this.configs.get("doku")!
    const baseUrl = config.baseUrl || "https://api.doku.com"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.post<DokuRefundResponse & { message?: string }>(
  ...
  private async refundIpaymu(transactionId: string, amount?: number, reason?: string): Promise<PaymentResponse> {
    const config = this.configs.get("ipaymu")!
    const baseUrl = config.baseUrl || "https://api.ipaymu.com/api/v2"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.post<IpaymuRefundResponse & { message?: string }>(
      ...
      if (data.status !== "success") {
        throw new PaymentGatewayError("ipaymu", data.message || "Refund failed", undefined, data as unknown as Record<string, unknown>)
      }
  ...
  private async refundNicepay(transactionId: string, amount?: number, reason?: string): Promise<PaymentResponse> {
    const config = this.configs.get("nicepay")!
    const baseUrl = config.baseUrl || "https://api.nicepay.co.id"
    const axios = getAxiosInstance()

    try {
      const { data } = await axios.post<NicepayRefundResponse & { message?: string }>(
        `${baseUrl}/api/v1/payment/refund`,
        {
          transactionId,
          amount,
