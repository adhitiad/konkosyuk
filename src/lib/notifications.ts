...
export async function sendWebPushNotification(userId: string, title: string, message: string) {
  ...
  const results = await Promise.allSettled(
    subscriptions.map(async (sub) => {
      try {
        ...
        if ((error as Error).message.includes('expired') || (error as Error).message.includes('410')) {
  ...
  if (failed > 0) {
    console.error(`[push] Failed to send ${failed}/${subscriptions.length} notifications`)
  }
}