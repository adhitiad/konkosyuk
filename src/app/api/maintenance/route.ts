...
export async function GET(req: NextRequest) {
  try {
    ...
    if (status) {
      whereClause = and(whereClause, eq(maintenanceTickets.status, status as (typeof maintenanceStatus)[number]))
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!activeBooking) {
      return fail('You can only create maintenance tickets for units you are currently renting', 400)
    }
