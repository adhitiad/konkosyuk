...
import { db } from '@/db'
import { payments, bookings, properties, users, platformSettings } from '@/db/schema'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalGMV = successPayments.reduce((sum, p) => sum + Number(p.amount), 0)
    const platformProfit = totalGMV * feePercent

    const ownerMap = new Map<string, { ownerId: string; ownerName: string; ownerEmail: string; totalEarning: number }>()
