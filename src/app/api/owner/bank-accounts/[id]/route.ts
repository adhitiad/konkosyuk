...
export async function GET(
  ...
  try {
    ...
    const [account] = await db
      ...
      .where(and(eq(ownerBankAccounts.id, id), eq(ownerBankAccounts.ownerId, session.user.id)))
      .limit(1)
...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db
      ...
      .set({
        ...(typeof body.is_primary === 'boolean' && { isPrimary: body.is_primary }),
...
export async function DELETE(
  ...
  try {
    ...
    if (account.isPrimary) {
      return fail('Tidak dapat menghapus rekening utama. Setel rekening lain sebagai utama terlebih dahulu.', 400)
    }
