...
export async function GET(req: NextRequest) {
  try {
    ...
    const bookedUnits = await db
      .select({ count: sql<number>`count(*)` })
      .from(units)
      .where(and(eq(units.propertyId, propertyIds[0]), eq(units.status, 'booked')))

    const occupancyRate = Number(totalUnits[0]?.count || 0) > 0 ? Number(bookedUnits[0]?.count || 0) / Number(totalUnits[0]?.count || 0) : 0
