...
export async function POST(req: NextRequest) {
  try {
    ...
    const type = (requestedType || 'avatar') as 'avatar' | 'property' | 'ktp' | 'report'
    ...
    if (type === 'ktp' && session.user.role !== 'owner') {
      return NextResponse.json({ error: 'Hanya owner yang dapat mengunggah KTP' }, { status: 403 })
    }
    if (type === 'property' && session.user.role !== 'owner') {
      return NextResponse.json({ error: 'Hanya owner yang dapat mengunggah properti' }, { status: 403 })
    }
