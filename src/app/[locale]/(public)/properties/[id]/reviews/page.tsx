...
export default function ReviewsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Review submitted', description: 'Thank you for your feedback!', type: 'success' })
      setNewRating(0)
      setComment('')
    },
    onError: (err) => {
      toast({ title: 'Gagal', description: err instanceof Error ? err.message : 'Gagal submit review.', type: 'error' })
    },
  })
  ...
  const handleSubmit = () => {
    ...
    createMutation.mutate({ bookingId: '', rating: newRating, comment: comment || undefined })
  }
  ...
  return (
    <div className="container py-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Lihat dan berikan review untuk properti ini</p>
      </div>
      ...
      {session && (
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={handleSubmit} disabled={newRating === 0 || createMutation.isPending}>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <p className="text-center text-muted-foreground py-8">Belum ada review untuk properti ini.</p>
        ) : (
          reviews.map((review) => (
            <Card key={review.id}>
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <StarRating rating={Number(review.rating)} readonly size="sm" />
                  </div>
