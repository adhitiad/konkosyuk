...
export default function BookingDialogClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedPackageId === "custom" && packages.custom.enabled && !customDuration) {
    ...
    try {
      ...
      showToastSuccess("Booking berhasil! Silakan bayar DP 35% untuk mengunci kamar.")
      setOpen(false)
      ...
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select<string> value={selectedPackageId} onValueChange={(v) => v && setSelectedPackageId(v)}>
              ...
              <SelectContent>
                {availablePackages.map((pkg) => (
                  <SelectItem key={pkg.id} value={pkg.id}>
                    {pkg.label} ({formatDuration(pkg.value, pkg.unit)}) - {formatCurrency(pkg.finalPrice)}
                  </SelectItem>
                ))}
          ...
          {selectedPackageId === "custom" && packages.custom.enabled && (
            <div className="space-y-2">
              <Label htmlFor="customDuration">Durasi ({packages.custom.unit})</Label>
              <Input
                id="customDuration"
                type="number"
              ...
              <p className="text-xs text-muted-foreground">
                Rentang: {packages.custom.minDuration} - {packages.custom.maxDuration} {packages.custom.unit}
              </p>
            </div>
          )}
          ...
          {selectedPackage && (
            <div className="rounded-4xl border p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span>+{selectedPackage.ppnPercent}% + {selectedPackage.appFeePercent}%</span>
              </div>
          ...
          <DialogFooter>
            <Button type="submit" disabled={loading || !selectedPackageId || !startDate}>
