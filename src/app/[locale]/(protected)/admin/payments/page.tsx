...
export default function AdminPaymentsPage() {
  ...
  const [manualProvider, setManualProvider] = useState<'doku' | 'ipaymu' | 'nicepay'>('doku')
  const [manualPurpose, setManualPurpose] = useState<'dp' | 'full_payment'>('dp')
  const [manualStatus, setManualStatus] = useState<'pending' | 'success'>('pending')

  const { data, isLoading, isError, error, refetch } = useQuery<PaymentResponse>({
  ...
  const reconcileMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Rekonsiliasi berhasil', description: 'Payment telah ditandai sebagai success.', type: 'success' })
      setReconcileId(null)
      setReconcileReason('')
      setReconcileTransactionId('')
    },
    onError: (err) => {
      toast({ title: 'Gagal', description: err instanceof Error ? err.message : 'Gagal merekonsiliasi payment.', type: 'error' })
    },
  })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-payments'] })
      toast({ title: 'Payment dibuat', description: 'Payment manual berhasil dibuat.', type: 'success' })
      setCreateOpen(false)
      setManualUserId('')
      setManualBookingId('')
      setManualAmount('')
    ...
    onError: (err) => {
      toast({ title: 'Gagal', description: err instanceof Error ? err.message : 'Gagal membuat payment manual.', type: 'error' })
    },
  })
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-payments'] })
      toast({ title: 'Payment dibatalkan', description: 'Payment telah dibatalkan.', type: 'success' })
      setCancelTarget(null)
      setCancelReason('')
    },
    onError: (err) => {
      toast({ title: 'Gagal', description: err instanceof Error ? err.message : 'Gagal membatalkan payment.', type: 'error' })
    },
  })
  ...
  const handleCreate = () => {
    if (!manualUserId || !manualBookingId || !manualAmount) {
      toast({ title: 'Gagal', description: 'UserId, BookingId, dan Amount wajib diisi.', type: 'error' })
      return
    }
  ...
  return (
    <div className="container py-6">
      ...
      <div className="mb-4">
        <Select<string> value={statusFilter} onValueChange={(v) => setStatusFilter(v ?? '')}>
                  ...
                  {payments.map((payment) => {
                    ...
                    return (
                      <tr key={payment.id} className="border-b last:border-b-0">
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">{formatCurrency(payment.amount)}</td>
                        <td className="py-3 px-4">
                          <Badge variant={config.variant}>{config.label}</Badge>
                        </td>
                        <td className="py-3 px-4 whitespace-nowrap">{formatDate(payment.paidAt)}</td>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            {payment.status === 'pending' && (
                              <Dialog>
                                <DialogTrigger render={
                                  <Button size="sm" variant="default" onClick={() => setReconcileId(payment.id)}>
                                    Mark as Success
                                  </Button>
                                } />
                                <DialogContent>
                                  ...
                                  {selectedPayment && selectedPayment.id === payment.id && (
                                    <div className="space-y-4">
                                      <div className="rounded-lg border p-3 space-y-1">
                                        <p className="text-sm"><span className="font-medium">Booking:</span> {selectedPayment.bookingCode?.slice(0, 8) ?? '-'}</p>
                                      ...
                                      <div className="flex justify-end gap-2">
                                        ...
                                        <Button
                                          disabled={reconcileMutation.isPending || !reconcileReason.trim()}
                                          onClick={handleReconcile}
                                        >
                                          {reconcileMutation.isPending ? 'Memproses...' : 'Konfirmasi'}
                                        </Button>
                                      </div>
                                    </div>
                                  )}
                            ...
                            {(payment.status === 'pending' || payment.status === 'success') && (
                              <Dialog>
                                <DialogTrigger render={
                                <Button size="sm" variant="destructive" onClick={() => setCancelTarget(payment)}>
                                  <HugeiconsIcon icon={MultiplicationSignIcon} strokeWidth={2} className="mr-1 size-4" />
                                  Batalkan
                                </Button>
                                } />
                                <DialogContent>
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="flex justify-end gap-2">
                                      <Button variant="outline" onClick={() => { setCancelTarget(null); setCancelReason('') }}>Batal</Button>
                                      <Button
                                        variant="destructive"
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={manualUserId} onChange={(e) => setManualUserId(e.target.value)} placeholder="UUID user" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Booking ID</label>
              <Input value={manualBookingId} onChange={(e) => setManualBookingId(e.target.value)} placeholder="UUID booking" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Amount (IDR)</label>
              <Input value={manualAmount} onChange={(e) => setManualAmount(e.target.value)} placeholder="Contoh: 350000" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Provider</label>
              <Select<string> value={manualProvider} onValueChange={(v) => setManualProvider(v as 'doku' | 'ipaymu' | 'nicepay')}>
            ...
            <div className="space-y-2">
              ...
              <Select<string> value={manualPurpose} onValueChange={(v) => setManualPurpose(v as 'dp' | 'full_payment')}>
                <SelectTrigger className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select<string> value={manualStatus} onValueChange={(v) => setManualStatus(v as 'pending' | 'success')}>
                <SelectTrigger className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
