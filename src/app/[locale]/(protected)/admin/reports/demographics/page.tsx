...
function AdminDemographicsPage() {
  ...
  useEffect(() => {
    const fetchProvinces = async () => {
      try {
        const data = await apiGet<{ id: string; name: string }[]>('/api/proxy/wilayah/provinces.json')
        setProvinces(data)
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchCities = async () => {
      try {
        const data = await apiGet<{ id: string; name: string }[]>(`/api/proxy/wilayah/regencies/${selectedProvince.id}.json`)
        setCities(data)
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchDistricts = async () => {
      try {
        const data = await apiGet<{ id: string; name: string }[]>(`/api/proxy/wilayah/districts/${selectedCity.id}.json`)
        setDistricts(data)
      } catch (err) {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<DemographicsResponse>({
    ...
    queryFn: async () => {
      ...
      const { data: json } = await apiClient.get(`/api/admin/reports/demographics?${params.toString()}`)
      return json.data
    },
    staleTime: 30000,
  })
  ...
  const top10Provinces = useMemo(() => provinceAggregation.slice(0, 10), [provinceAggregation])
  const otherProvinces = useMemo(
    () => ({
      name: 'Lainnya',
      value: provinceAggregation.slice(10).reduce((sum, item) => sum + item.value, 0),
    }),
    [provinceAggregation]
  )

  const pieData = useMemo(() => {
    const combined = otherProvinces.value > 0 ? [...top10Provinces, otherProvinces] : top10Provinces
    return combined
  ...
  return (
    <div className="container py-6 space-y-6">
      <div className="mb-4">
        ...
        <p className="text-muted-foreground">Distribusi user dan owner berdasarkan wilayah</p>
      </div>
      ...
      {isError && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {error instanceof Error ? error.message : 'Gagal memuat data demografi.'}
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            <Select value={filterType} onValueChange={(v) => { setFilterType(v as 'user' | 'owner'); setProvince(''); setCity(''); setDistrict(''); }}>
            ...
            <Select value={city} onValueChange={(v) => { setCity(v ?? ''); setDistrict(''); }} disabled={!province}>
              <SelectTrigger className="w-56">
                <SelectValue placeholder={province ? 'Semua Kota/Kabupaten' : 'Pilih provinsi dahulu'} />
              </SelectTrigger>
              <SelectContent>
            ...
            <Select value={district} onValueChange={(v) => setDistrict(v ?? '')} disabled={!city || filterType === 'owner'}>
              <SelectTrigger className="w-56">
                <SelectValue placeholder={filterType === 'owner' ? 'Tidak tersedia untuk Owner' : (city ? 'Semua Kecamatan' : 'Pilih kota dahulu')} />
              </SelectTrigger>
              <SelectContent>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              {isLoading ? <Skeleton className="h-8 w-20" /> : data?.data.length ?? 0}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="map" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Peta Sebaran {filterType === 'user' ? 'User' : 'Owner'} di Indonesia</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                <Skeleton className="h-[600px] w-full" />
              ) : (
                <IndonesiaMapVisualization data={data?.data || []} filterType={filterType} />
              )}
            </CardContent>
          </Card>
        </TabsContent>
              ...
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <PieChart>
                    ...
                    <Tooltip formatter={(value: any) => [`${value} ${filterType === 'user' ? 'User' : 'Owner'}`, 'Jumlah']} />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {filterType === 'user' && <TableHead scope="col">Kecamatan</TableHead>}
                ...
                <TableBody>
                  {data.data.map((item, idx) => (
                    <TableRow key={idx}>
                      ...
                      {filterType === 'user' && <TableCell>{item.district}</TableCell>}
