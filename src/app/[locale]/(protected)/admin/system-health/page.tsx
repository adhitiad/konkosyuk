...
function SystemHealthPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <Activity className="size-5 animate-pulse text-muted-foreground" />
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        <BreadcrumbNav items={[{ label: 'Dashboard', href: '/admin' }, { label: 'System Health' }]} />
        <h1 className="text-3xl font-bold tracking-tight">System Health</h1>
        <p className="text-muted-foreground">
          Last checked: {data?.lastChecked ? new Date(data.lastChecked).toLocaleTimeString('id-ID') : '-'}
        </p>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {data?.checks.map((check) => (
                <div key={check.name} className="flex items-center justify-between rounded-4xl border p-4">
