...
function AdminBookingsPage() {
  ...
  const { data: bookings = [], isLoading } = useQuery<Booking[]>({
    ...
    queryFn: async () => {
      ...
      return items.map((b) => ({
        ...
        totalPrice: (b.metadata as Record<string, unknown> | undefined)?.totalPrice ? String((b.metadata as Record<string, unknown>).totalPrice) : '0',
      }))
    },
    staleTime: 30000,
    enabled: !!session,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="mb-6">
        <BreadcrumbNav items={[{ label: 'Dashboard', href: '/admin' }, { label: 'Manajemen Booking' }]} />
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">Tidak ada booking</p>
          ) : (
