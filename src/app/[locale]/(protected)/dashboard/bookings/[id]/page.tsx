...
const statusConfig: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; icon: React.ReactNode }> = {
  pending_dp: { label: 'Menunggu Bayar DP', variant: 'secondary', icon: <HugeiconsIcon icon={Clock01Icon} strokeWidth={2} className="size-4" /> },
  awaiting_owner_approval: { label: 'Menunggu Persetujuan Owner', variant: 'default', icon: <HugeiconsIcon icon={Clock01Icon} strokeWidth={2} className="size-4" /> },
  awaiting_full_payment: { label: 'Menunggu Pelunasan', variant: 'default', icon: <HugeiconsIcon icon={Clock01Icon} strokeWidth={2} className="size-4" /> },
  confirmed: { label: 'Dikonfirmasi', variant: 'default', icon: <HugeiconsIcon icon={CheckmarkCircle02Icon} strokeWidth={2} className="size-4" /> },
  rejected: { label: 'Ditolak', variant: 'destructive', icon: <HugeiconsIcon icon={CancelCircleIcon} strokeWidth={2} className="size-4" /> },
  cancelled: { label: 'Dibatalkan', variant: 'destructive', icon: <HugeiconsIcon icon={CancelCircleIcon} strokeWidth={2} className="size-4" /> },
}

const statusSteps = ['pending_dp', 'awaiting_owner_approval', 'awaiting_full_payment', 'confirmed']
...
export default function BookingDetailPage() {
    ...
    return (
      <div className="container py-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {error instanceof Error ? error.message : 'Booking tidak ditemukan.'}
          </AlertDescription>
        </Alert>
      </div>
    )
  ...
  const config = statusConfig[booking.status] ?? { label: booking.status, variant: 'outline', icon: null }
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="text-sm font-medium capitalize">{booking.bookingType}</p>
              </div>
              <div>
            ...
            <CardContent>
              <div className="relative flex items-center justify-between">
                ...
                {statusSteps.map((step, index) => {
                  const isCompleted = index < currentStepIndex || booking.status === 'confirmed'
                    ...
                    <div key={step} className="relative flex flex-col items-center gap-2">
                      ...
                      >
                        {isCompleted && !isRejected ? (
                          <HugeiconsIcon icon={CheckmarkCircle02Icon} strokeWidth={2} className="size-4 text-primary-foreground" />
                        ) : isRejected ? (
              ...
              {booking.payments.length === 0 ? (
                ...
                <div className="space-y-4">
                  {booking.payments.map((payment) => (
                    <div key={payment.id} className="flex flex-col gap-2 rounded-xl border p-4">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium capitalize">{payment.purpose.replace(/_/g, ' ')}</span>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {booking.status === 'pending_dp' && (
                <Button render={<Link href={`/dashboard/bookings/${booking.id}/checkout?purpose=dp`} />} className="w-full" nativeButton={false}>
                  Bayar DP
                </Button>
              )}
              {booking.status === 'awaiting_full_payment' && (
                <Button render={<Link href={`/dashboard/bookings/${booking.id}/checkout?purpose=full_payment`} />} className="w-full" nativeButton={false}>
                  Bayar Pelunasan
                </Button>
              )}
              {booking.status === 'confirmed' && (
                <Button render={<Link href={`/properties/${booking.propertyId}`} />} variant="outline" className="w-full" nativeButton={false}>
                  Lihat Properti
                </Button>
              )}
              {(booking.status === 'rejected' || booking.status === 'cancelled') && (
                <Button render={<Link href="/properties" />} variant="outline" className="w-full" nativeButton={false}>
                  Cari Properti Lain
                </Button>
              )}
            </CardContent>
