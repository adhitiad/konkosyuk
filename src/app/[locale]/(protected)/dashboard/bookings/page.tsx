...
import { AlertCircleIcon, CheckmarkCircle02Icon, Clock01Icon, CancelCircleIcon, Search01Icon } from '@hugeicons/core-free-icons'
...
export default function TenantBookingsPage() {
  ...
  const { data, isLoading, isError, error } = useQuery<{ data: BookingRequestItem[] }>({
  ...
  return (
    <div className="container py-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Lihat status permintaan sewa Anda</p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            />
            <h3 className="text-xl font-semibold mb-2">Belum ada riwayat booking</h3>
            <p className="text-sm text-muted-foreground mb-6 max-w-md">
              Anda belum pernah melakukan permintaan sewa. Jelajahi properti tersedia dan temukan kamar ideal Anda.
            </p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {bookings.map((booking) => {
            ...
            const price = booking.matchedPrice ?? (booking.agreedPrice ? Number(booking.agreedPrice) : null)
              ...
              <Card key={booking.id} className="flex flex-col">
                ...
                <CardContent className="flex flex-1 flex-col gap-3">
                  <div className="space-y-1">
                    <p className="text-sm">
                      <span className="text-muted-foreground">Jumlah Penghuni:</span>{' '}
                      <span className="font-medium">{booking.numOccupants} orang</span>
                    </p>
                    <p className="text-sm">
                  ...
                  {booking.status === 'approved' && price !== null && (
                    <div className="rounded-4xl border p-3 space-y-2">
                      <p className="text-xs text-muted-foreground">DP 35% yang harus dibayar:</p>
                  ...
                  <div className="mt-auto flex items-center justify-end gap-2">
                    {booking.status === 'approved' && (
                      <Button size="sm" render={<Link href={`/dashboard/bookings/${booking.id}`} />} nativeButton={false}>
                        Bayar Sekarang
                      </Button>
                    )}
                  </div>
