...
const DEFAULT_UNIT_IMAGE = 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=400&q=80'
...
export default function UnitCard({ unit, onSelect, action }: UnitCardProps) {
  ...
  const imageUrl = unit.images && unit.images.length > 0 ? unit.images[0] : DEFAULT_UNIT_IMAGE
    ...
    <Card className="flex h-full flex-col overflow-hidden">
      ...
      <CardHeader className="flex-1">
        ...
        <p className="text-sm font-semibold text-primary">
          {formatCurrency(price)}<span className="text-xs text-muted-foreground">/bulan</span>
        </p>
      </CardHeader>
      {unit.description && (
        <CardContent>
          <p className="line-clamp-2 text-xs text-muted-foreground">{unit.description}</p>
        </CardContent>
      )}
