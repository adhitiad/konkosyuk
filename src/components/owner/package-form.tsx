...
function calculateFinalPrice(basePrice: number, discountPercent: number, ppnPercent: number, appFeePercent: number): number {
...
export default function PackageForm({ type, initialData, onChange }: PackageFormProps) {
  const defaultPackages: PropertyPackages = useMemo(() => ({
    predefined: (type === 'kost' ? KOST_PACKAGES : KONTRAKAN_PACKAGES).map((pkg) => ({
      ...pkg,
      basePrice: 0,
      finalPrice: 0,
    })),
  ...
  const [packages, setPackages] = useState<PropertyPackages>(initialData ?? defaultPackages)

  useEffect(() => {
    onChange(packages)
  }, [packages, onChange])

  const updatePackage = (index: number, field: keyof PackageItem, value: PackageItem[keyof PackageItem]) => {
    setPackages((prev) => {
      const updated = [...prev.predefined]
      updated[index] = { ...updated[index], [field]: value }
      
      if (field === 'basePrice' || field === 'discountPercent' || field === 'ppnPercent' || field === 'appFeePercent') {
        const pkg = updated[index] as PackageItem
        updated[index] = {
          ...pkg,
          finalPrice: calculateFinalPrice(pkg.basePrice, pkg.discountPercent, pkg.ppnPercent, pkg.appFeePercent),
        }
      }
  ...
  const updateCustom = (field: string, value: PropertyPackages['custom'][keyof PropertyPackages['custom']]) => {
    setPackages((prev) => ({
      ...prev,
            ...
            <div key={pkg.id} className="rounded-4xl border p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">({formatDuration(pkg.value, pkg.unit)})</span>
                </div>
                <Switch
              ...
              {pkg.isAvailable && (
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label htmlFor={`price-${pkg.id}`} className="text-xs">Harga Dasar</Label>
                    <Input
                      id={`price-${pkg.id}`}
                      type="number"
                      value={pkg.basePrice || ''}
                      onChange={(e) => updatePackage(index, 'basePrice', Number(e.target.value))}
                      placeholder="0"
                      min="0"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor={`discount-${pkg.id}`} className="text-xs">Diskon (%)</Label>
                    <Input
                      id={`discount-${pkg.id}`}
                      type="number"
                      value={pkg.discountPercent || ''}
                      onChange={(e) => updatePackage(index, 'discountPercent', Number(e.target.value))}
                      placeholder="0"
                      min="0"
                      max="100"
                    />
                  ...
                  <div className="space-y-1">
                    <Label htmlFor={`ppn-${pkg.id}`} className="text-xs">PPN (%)</Label>
                    <Input
                      id={`ppn-${pkg.id}`}
                      type="number"
                      value={pkg.ppnPercent || ''}
                      onChange={(e) => updatePackage(index, 'ppnPercent', Number(e.target.value))}
                      placeholder="11"
                      min="0"
                      max="100"
                    />
                  ...
                  <div className="space-y-1">
                    <Label htmlFor={`app-${pkg.id}`} className="text-xs">Biaya App (%)</Label>
                    <Input
                      id={`app-${pkg.id}`}
                      type="number"
                      value={pkg.appFeePercent || ''}
                      onChange={(e) => updatePackage(index, 'appFeePercent', Number(e.target.value))}
                      placeholder="0.63"
                      min="0"
                      max="100"
                      step="0.01"
              ...
              {pkg.isAvailable && pkg.basePrice > 0 && (
                <div className="text-sm text-muted-foreground">
                  Harga Final: <span className="font-semibold text-foreground">{formatCurrency(pkg.finalPrice)}</span>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="space-y-3 rounded-4xl border p-4">
        ...
        {packages.custom.enabled && (
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <Label htmlFor="custom-unit" className="text-xs">Satuan</Label>
              <select
                id="custom-unit"
                value={packages.custom.unit}
                onChange={(e) => updateCustom('unit', e.target.value as DurationUnit)}
            ...
            <div className="space-y-1">
              <Label htmlFor="custom-price" className="text-xs">Harga per Unit</Label>
              <Input
                id="custom-price"
                type="number"
                value={packages.custom.pricePerUnit || ''}
                onChange={(e) => updateCustom('pricePerUnit', Number(e.target.value))}
                placeholder="0"
                min="0"
              />
            </div>
            <div className="space-y-1">
              <Label htmlFor="custom-min" className="text-xs">Min Durasi</Label>
              <Input
                id="custom-min"
                type="number"
                value={packages.custom.minDuration || ''}
                onChange={(e) => updateCustom('minDuration', Number(e.target.value))}
                placeholder="1"
                min="1"
              />
            </div>
            <div className="space-y-1">
              <Label htmlFor="custom-max" className="text-xs">Max Durasi</Label>
              <Input
                id="custom-max"
                type="number"
                value={packages.custom.maxDuration || ''}
                onChange={(e) => updateCustom('maxDuration', Number(e.target.value))}
                placeholder="365"
                min="1"
              />
            </div>
