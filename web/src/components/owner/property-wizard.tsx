...
import { createPropertyWithUnitsAction } from "@/actions/properties";
...
export default function PropertyWizard() {
  ...
  const { data: tagsData } = useQuery<Tag[]>({
    queryKey: ["tags"],
    queryFn: async () => {
      const { data } = await (await import("@/lib/axios")).apiClient.get("/api/owner/tags");
  ...
  const availableCities = step1.province
    ? CITIES_BY_PROVINCE[step1.province] || []
    : [];
  const imageCount = step1.images.length;
  const canProceedStep1 =
    step1.title.trim().length > 0 &&
    step1.address.trim().length >= 10 &&
    step1.province &&
    step1.city &&
    imageCount >= 3;
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      for (const unit of units) {
        const enriched = await uploadUnitImages(
          unit,
          unitFilesMap[unit._tempId] || [],
        );
        enrichedUnits.push(enriched);
      }
      ...
      const result = await createPropertyWithUnitsAction(undefined, formData);
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <div
            className={`flex size-8 items-center justify-center rounded-full text-sm font-medium ${step === 1 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            1
          </div>
          <span
        ...
        <div className="flex items-center gap-2">
          <div
            className={`flex size-8 items-center justify-center rounded-full text-sm font-medium ${step === 2 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            2
          </div>
          <span
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={step1.type}
                onValueChange={(v) =>
                  updateStep1({ type: v as Step1Data["type"] })
                }
              >
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger id="city">
                  <SelectValue
                    placeholder={
                      step1.province ? "Pilih Kota" : "Pilih Provinsi dahulu"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
      ...
      {step === 2 && (
        <div className="space-y-6">
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Dropzone
                        onFilesChange={(files) => {
                          setUnitFilesMap((prev) => ({
                            ...prev,
                            [editingUnit?._tempId || "new"]: files,
                          }));
                        }}
                        maxFiles={5}
                        minFiles={0}
                        currentFiles={
                          unitFilesMap[editingUnit?._tempId || "new"] || []
                        }
                      />
                    </div>
                    <div className="flex justify-end gap-2">
                      ...
                      <Button
                        onClick={saveUnit}
                        disabled={!unitForm.name.trim() || !unitForm.price}
                      >
            ...
            <CardContent>
              {units.length === 0 ? (
                <div className="py-8 text-center text-sm text-muted-foreground">
                  Belum ada unit. Klik &ldquo;Tambah Kamar/Unit&rdquo; untuk menambah.
                </div>
              ) : (
                    ...
                    <Card key={unit._tempId} size="sm">
                      <CardHeader className="flex flex-row items-start justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Rp {Number(unit.price).toLocaleString("id-ID")} /
                            bulan
                          </p>
                        </div>
                        <Badge
                      ...
                      <div className="flex items-center justify-end gap-2 px-(--card-spacing) pb-(--card-spacing)">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => removeUnit(unit._tempId)}
                        >
                          <HugeiconsIcon
                            icon={Delete01Icon}
                            strokeWidth={2}
          ...
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleSaveAll} disabled={isSubmitting || units.length === 0}>
