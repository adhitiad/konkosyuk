  ...
  try {
    ...
    if (body.unitId) {
      const [unit] = await db
        ...
        .from(units)
        .where(
          and(eq(units.id, body.unitId), eq(units.propertyId, body.propertyId)),
        )
        .limit(1);
      ...
      recipients.flatMap((recipient) => [
        ...
        recipient.phone || recipient.whatsapp
          ? sendMaintenanceWhatsApp(
              ...
              [recipient.name, property.name, body.category, body.description],
            )
          : Promise.resolve(),
      ]),
    );
...
export async function GET(req: NextRequest) {
  try {
    ...
    const conditions = [];
    if (propertyId)
      conditions.push(eq(maintenanceReports.propertyId, propertyId));
    if (
      status &&
      maintenanceReportStatus.includes(
        status as (typeof maintenanceReportStatus)[number],
      )
    )
      conditions.push(
        eq(
          maintenanceReports.status,
          status as (typeof maintenanceReportStatus)[number],
        ),
      );
    if (session.user.role === "owner")
      conditions.push(eq(properties.ownerId, session.user.id));

    const data = await db
      .select({
        id: maintenanceReports.id,
        tenantId: maintenanceReports.tenantId,
        propertyId: maintenanceReports.propertyId,
        unitId: maintenanceReports.unitId,
        category: maintenanceReports.category,
        description: maintenanceReports.description,
        images: maintenanceReports.images,
        status: maintenanceReports.status,
        resolutionNote: maintenanceReports.resolutionNote,
        createdAt: maintenanceReports.createdAt,
        updatedAt: maintenanceReports.updatedAt,
