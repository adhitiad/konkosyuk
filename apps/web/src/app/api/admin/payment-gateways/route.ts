...
export async function GET() {
  try {
    ...
    const credentialByGateway = new Map(
      credentials.map((credential) => [credential.gatewayId, credential]),
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validation =
      providerConfigSchemas[body.provider].safeParse(mergedConfig);
    if (!validation.success)
      return fail(
        validation.error.issues[0]?.message ?? "Invalid gateway configuration",
        400,
      );
