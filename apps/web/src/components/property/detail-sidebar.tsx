...
export function DetailSidebar({
  ...
  return (
    <div className="w-full lg:w-[340px] lg:sticky lg:top-20 lg:self-start space-y-4">
      ...
      {owner && <OwnerProfileCard owner={owner} propertyId={propertyId} />}

      {rules.length > 0 && <PropertyRulesList rules={rules} />}

      {nearbyPlaces.length > 0 && <NearbyPlacesList places={nearbyPlaces} />}
    </div>
  );
}
