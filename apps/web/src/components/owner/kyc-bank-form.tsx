...
import type { AddBankAccountInput } from "@konkosyuk/shared";
import {
  showToastSuccess,
  showToastError,
  showToastWarning,
...
export function KYCBankForm({
  ...
  const availableProviders =
    PROVIDERS.find((p) => p.value === accountType)?.options ?? [];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <Select
          value={accountType}
          onValueChange={(value) => setAccountType(value as "bank" | "ewallet")}
        >
